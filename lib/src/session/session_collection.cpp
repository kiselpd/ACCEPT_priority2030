...
// SessionCollection
SessionCollection::SessionCollection() : collection_(std::make_shared<std::vector<std::shared_ptr<BaseSession>>>()){};
...
std::vector<std::shared_ptr<BaseSession>>::iterator SessionCollection::findSession(const std::string &id, const User &owner)
{
